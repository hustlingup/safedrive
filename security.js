...
const SecurityModule = {
  ...
  async getCanvasFingerprint() {
    try {
      ...
      ctx.fillStyle = "#069";
      ctx.fillText("SafeDrive ?š—", 2, 15);
      ctx.fillStyle = "rgba(102, 204, 0, 0.7)";
      ctx.fillText("SafeDrive ?š—", 4, 17);
  ...
  async secureIncrementCounter(plateNumber, counterKey) {
    try {
      ...
      let userMessage = "ë©”ì„¸ì§€ ?„ì†¡???¤íŒ¨?ˆìŠµ?ˆë‹¤";
      
      if (error.code === "functions/resource-exhausted") {
        userMessage = "?ˆë¬´ ë§Žì? ?”ì²­??ë°œìƒ?ˆìŠµ?ˆë‹¤. ? ì‹œ ???¤ì‹œ ?œë„??ì£¼ì„¸??;
      } else if (error.code === "functions/permission-denied") {
        userMessage = "?”ì²­??ê±°ë??˜ì—ˆ?µë‹ˆ??;
      } else if (error.code === "functions/failed-precondition") {
        userMessage = "?”ì²­??ë§Œë£Œ?˜ì—ˆ?µë‹ˆ?? ?¤ì‹œ ?œë„??ì£¼ì„¸??;
      } else if (error.code === "functions/invalid-argument") {
        userMessage = "?˜ëª»???”ì²­?…ë‹ˆ??;
      } else if (error.message && error.message.includes("network")) {
        userMessage = "?¤íŠ¸?Œí¬ ?¤ë¥˜ê°€ ë°œìƒ?ˆìŠµ?ˆë‹¤. ?¤ì‹œ ?œë„??ì£¼ì„¸??;
      }
